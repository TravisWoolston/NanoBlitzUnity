...
public class VectorGrid : MonoBehaviour 
{
	...
	// public static VectorGrid Instance;
...
/// </summary>
	...
	{	
		float gridWidth = 1.0f + (1.0f / (m_GridWidth - 1));
		float gridHeight = 1.0f + (1.0f / (m_GridHeight - 1));
		if(amount != Vector2.zero)
		{
			...
			while(m_ScrollAmount.x > m_GridScale)
			{
				...
				if(m_RenderMode == RenderMode.Plane)
				{
					...
					for(int y = 0; y < m_GridHeight; y++)
					{
						m_UVs[(m_GridHeight * xIndex) + y].x -= gridWidth;
					}
			...
			while(m_ScrollAmount.y > m_GridScale)
			{
				...
				if(m_RenderMode == RenderMode.Plane)
				{
					...
					for(int x = 0; x < m_GridWidth; x++)
					{
						m_UVs[(m_GridHeight * x) + yIndex].y -= gridHeight;
					}
